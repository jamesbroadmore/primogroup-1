...
import { extractPerthTime, perthToISO } from "@/lib/perth-time";
...
interface ShiftData {
  ...
  client?: { first_name: string; last_name: string; preferred_name?: string | null } | null;
}
...
export function EditShiftDialog({ shift, onClose, staffList, clientList }: EditShiftDialogProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.staffId) { toast.error("Staff member is required"); return; }
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div className="flex items-center gap-2">
            ...
            {isAdmin && form.status === "pending" && (
              <button type="button" onClick={() => setForm({ ...form, status: "approved" })}
                className="text-[10px] text-success hover:underline font-medium">Approve</button>
            )}
          </div>

          <div>
            <label className="text-xs font-medium text-muted-foreground mb-1.5 block">Date</label>
            <input type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} disabled={!isAdmin}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="time" value={form.startTime} onChange={(e) => setForm({ ...form, startTime: e.target.value })} disabled={!isAdmin}
                className="w-full h-9 rounded-lg border bg-background px-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring disabled:opacity-60" />
            </div>
            <div>
              <label className="text-xs font-medium text-muted-foreground mb-1.5 block">End Time</label>
              <input type="time" value={form.endTime} onChange={(e) => setForm({ ...form, endTime: e.target.value })} disabled={!isAdmin}
                className="w-full h-9 rounded-lg border bg-background px-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring disabled:opacity-60" />
            </div>
          </div>
          ...
          <div>
            ...
            <select value={form.staffId} onChange={(e) => setForm({ ...form, staffId: e.target.value })} disabled={!isAdmin}
              className="w-full h-9 rounded-lg border bg-background px-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring disabled:opacity-60">
          ...
          <div>
            ...
            <select value={form.clientId} onChange={(e) => setForm({ ...form, clientId: e.target.value })} disabled={!isAdmin}
              className="w-full h-9 rounded-lg border bg-background px-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring disabled:opacity-60">
          ...
          <div>
            ...
            <textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} placeholder="Shift notes..." rows={2} disabled={!isAdmin}
          ...
          {isAdmin && (
            <div className="flex justify-between pt-2">
              <button type="button" onClick={() => { if (confirm("Delete this shift?")) deleteMutation.mutate(); }} disabled={isPending}
          ...
          {!isAdmin && (
            <div className="flex justify-end pt-2">
              <button type="button" onClick={onClose} className="h-9 px-4 rounded-lg border text-sm font-medium text-foreground hover:bg-secondary transition-colors">Close</button>
            </div>
          )}
        </form>
      </motion.div>
