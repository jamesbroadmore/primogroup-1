      ...
      >
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <DetailItem icon={User} label="Staff Member" value={fullName(t.staff)} />
            <DetailItem icon={UserCircle} label="Client" value={fullName(t.client)} />
            <DetailItem icon={Calendar} label="Shift Date" value={formatPerthDate(t.shift_date)} />
            <DetailItem icon={Clock} label="Hours Worked" value={t.total_hours ? `${t.total_hours}h` : "—"} />
          </div>
          ...
          <div className="rounded-lg bg-secondary/50 p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <p className="text-sm font-semibold text-card-foreground">{t.start_time ? formatPerthTime(t.start_time) : "—"}</p>
              </div>
              <div>
                <p className="text-[10px] text-muted-foreground">End</p>
                <p className="text-sm font-semibold text-card-foreground">{t.end_time ? formatPerthTime(t.end_time) : "—"}</p>
              </div>
              <div>
                <p className="text-[10px] text-muted-foreground">Break</p>
                <p className="text-sm font-semibold text-card-foreground">{t.break_minutes != null ? `${t.break_minutes} min` : "0 min"}</p>
              </div>
              <div>
                <p className="text-[10px] text-muted-foreground">Rate</p>
                <p className="text-sm font-semibold text-card-foreground">{t.rate_per_hour ? `$${Number(t.rate_per_hour).toFixed(2)}/hr` : "—"}</p>
              </div>
            </div>
          </div>
        ...
        <div className="flex justify-end p-5 border-t">
          <button onClick={onClose} className="h-9 px-4 rounded-lg border text-sm font-medium text-foreground hover:bg-secondary transition-colors">Close</button>
        </div>
      </motion.div>
    </div>
  );
